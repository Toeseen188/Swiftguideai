...
export async function getDisasterData(lat, lng, disasterType, location) {
  console.log('[SwiftRoute] USE_REAL_DATA env value:', 
    process.env.USE_REAL_DATA);
  console.log('[SwiftRoute] USE_REAL evaluated as:', 
    process.env.USE_REAL_DATA === 'true');

  const USE_REAL = process.env.USE_REAL_DATA === 'true';
  
  if (!USE_REAL) {
    console.log('[SwiftRoute] 🟡 Mock mode — set USE_REAL_DATA=true to use live data');
  ...
  try {
    ...
    console.warn('[SwiftRoute] ❌ Real data failed, falling back:', err.message);
    return { ...getMockDisasterData(lat, lng, disasterType), dataSource: 'mock' };
  }
}
